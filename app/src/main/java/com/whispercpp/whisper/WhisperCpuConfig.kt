...
object WhisperCpuConfig {
    val preferredThreadCount: Int
        get() {
            // Get count of high-performance cores to prevent thread scheduling on efficiency cores
            val highPerfCores = CpuInfo.getHighPerfCpuCount()
            return if (highPerfCores > 0) {
                highPerfCores.coerceIn(2, 4)
            } else {
...
private class CpuInfo(private val lines: List<String>) {
    ...
    private fun getHighPerfCpuCountByFrequencies(): Int = getCpuValues(property = "processor") {
        getMaxCpuFrequency(it.toInt())
    }
        .also { Log.d(LOG_TAG, "Binned cpu frequencies (frequency, count): ${it.binnedValues()}") }
        .countDroppingMin()

    private fun getHighPerfCpuCountByVariant(): Int = getCpuValues(property = "CPU variant") {
        it.substringAfter("0x").toInt(radix = 16)
    }
