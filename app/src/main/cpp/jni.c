...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_fullTranscribe(
    ...
    for (int i = 0; i < audio_data_length; i++) {
        float val = audio_data_arr[i] < 0 ? -audio_data_arr[i] : audio_data_arr[i];
        if (val > max_val) max_val = val;
    }

    LOGI("About to run whisper_full with %d samples. Max amp: %f", audio_data_length, max_val);
    if (whisper_full(context, params, audio_data_arr, audio_data_length) != 0) {
        LOGI("Failed to run the model");
    } else {
        LOGI("whisper_full finished successfully");
